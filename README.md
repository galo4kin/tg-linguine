...
| `MAX_TOKENS_PER_ARTICLE` | `10000` | Гейт по эвристике `runes/4`. Если статья длиннее — бот предлагает выбрать: разобрать первую часть (truncation) или сжать перед разбором (LLM pre-summary). Дефолт подобран под Groq free tier, который 413-ит запросы выше ~10–12K токенов даже на моделях с 128K контекстом. На paid tier этот лимит можно поднимать (например до 30000). |
