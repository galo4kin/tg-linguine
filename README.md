...
| `QUIZ_DAILY_GOAL` | `20` | Сколько правильных ответов в день нужно для бонуса XP. Сбрасывается на стыке суток. |
| `QUIZ_XP_PER_CORRECT` | `10` | XP за один правильный ответ в `/study`. |
| `QUIZ_XP_BONUS_GOAL` | `50` | Дополнительный XP за достижение дневной цели (выплачивается ровно один раз в день). |
...
| `/study` | Тренировка слов из статуса `learning` — multiple-choice квиз (inline-кнопки и нативные quiz-опросы Telegram чередуются 50/50). После каждого ответа: фидбэк с `+XP`, при достижении дневной цели — одноразовый бонус. | все |
| `/me` | Профиль: учимый язык/уровень, общий XP, дневной стрик и прогресс к дневной цели | все |
