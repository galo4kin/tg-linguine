...
func (a *Admin) HandleStats(ctx context.Context, b *bot.Bot, update *models.Update) {
	...
	if err != nil {
		a.log.Error("admin /stats: users.Stats", "errors_total", 1, "err", err)
		return
	}
	totalArticles, err := a.articles.CountAll(ctx, a.db)
	if err != nil {
		a.log.Error("admin /stats: articles.CountAll", "errors_total", 1, "err", err)
		return
	}
	dayArticles, err := a.articles.CountSince(ctx, a.db, time.Now().Add(-24*time.Hour))
	if err != nil {
		a.log.Error("admin /stats: articles.CountSince", "errors_total", 1, "err", err)
		return
	}
	totalWords, err := a.dict.CountAll(ctx, a.db)
	if err != nil {
		a.log.Error("admin /stats: dict.CountAll", "errors_total", 1, "err", err)
		return
	}
